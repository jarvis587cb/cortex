...
- **Behoben:** **Store-Migration:** Die `CREATE INDEX IF NOT EXISTS`-Aufrufe nach der AutoMigrate wurden ohne Fehlerprüfung ausgeführt. Schlägt ein Index-Aufbau fehl (z. B. Speicher/Rechte), startete der Server trotzdem ohne Indizes. Die Migration bricht nun bei Fehler ab und gibt den Fehler zurück. Dadurch teilten sich alle Clients hinter demselben Proxy ein Limit. Es wird nun die linke (clientseitige) Adresse verwendet: `strings.TrimSpace(strings.Split(forwarded, ",")[0])`. Bei HTTP Keep-Alive kann das die Wiederverwendung der Verbindung verhindern. Beim Schließen wird der Body nun mit `io.Copy(io.Discard, resp.Body)` geleert. Ungültige oder unsichere Schemas (z. B. `javascript:`, `ftp://`) wurden gespeichert und führten erst beim Zustellversuch zu Fehlern. Es gibt nun `helpers.ValidateWebhookURL` (nicht leer, parsebar, Schema http/https, Host gesetzt). Test: `TestValidateWebhookURL`.
