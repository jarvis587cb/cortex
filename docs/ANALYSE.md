...
- **Behoben:** **Webhook-URL:** Beim Anlegen eines Webhooks wurde die URL nicht validiert. Ungültige oder unsichere Schemas (z. B. `javascript:`, `ftp://`) wurden gespeichert und führten erst beim Zustellversuch zu Fehlern. Es gibt nun `helpers.ValidateWebhookURL` (nicht leer, parsebar, Schema http/https, Host gesetzt). Test: `TestValidateWebhookURL`.
